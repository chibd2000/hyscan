...
1)遇到的情况，本地组弱口令自动枚举改了点代码，忽略了一种情况就是环境中的管理员组不一定是"administrators"，有可能是"admins"等等之类的，这里用samr相关的方法来进行获取，通过固定的RID 544来获取就可以解决了

2022.1.31变动

1)遇到的情况，域中添加的机器函数有时候会有问题，原因是缓冲区大小太小，有时候遇到机器名长的就有可能获取失败，这里修改了下
